...
def generate_title(user_message: str, assistant_message: str):
    ...
    prompt = ChatPromptTemplate([
        ("system", "次のユーザ(user)とLLMの回答(assistant)のやり取りに20文字以内でタイトルをつけてください。メッセージは次の形式で来ます: user=今日の天気は？, assistant=今日の天気は晴れです。"),
    ...
    return chain.invoke({"user_message": user_message, "assistant_message": assistant_message})